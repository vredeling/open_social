...
function activity_creator_post_update_8802_remove_orphaned_activities(&$sandbox) {
  ...
  if (!isset($sandbox['activities_id'])) {
    ...
    if (empty($sandbox['count'])) {
      $sandbox['#finished'] = 1;
      \Drupal::messenger()->addMessage("No entities to be processed.");
      return;
    }
  ...
  \Drupal::messenger()->addMessage('Progress: ' . (round($progress_fraction * 100)) . '% (' . $sandbox['progress'] . ' of ' . $sandbox['count'] . ' activities processed)');
